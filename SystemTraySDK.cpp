...
LRESULT PASCAL CSystemTray::WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    //    if (pTrayIcon->GetSafeHwnd() != hWnd) continue;
          ...
          if (message == WM_COMMAND)
              ::PostMessage(pTrayIcon->m_hTargetWnd, message, wParam, lParam);

    //    pTrayIcon = GetNextTrayIcon();
    // }

    //CString msgDump;
    //msgDump.Format(_T("Tray Icon Message %08X(WM_COMMAND=%04X), wParam = 0x%llX, lParam = 0x%llX\n"), message, WM_COMMAND, wParam, lParam);
    //::OutputDebugString(msgDump);
